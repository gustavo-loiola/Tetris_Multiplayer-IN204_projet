...
void ScoreManager::addSoftDropCells(int cells)
{
    if (cells <= 0) return;
    score_ += static_cast<std::uint64_t>(cells); // +1 per cell
}

void ScoreManager::addHardDropCells(int cells)
{
    if (cells <= 0) return;
    score_ += static_cast<std::uint64_t>(2 * cells); // +2 per cell
}
